...
namespace EngineHelpers
{
    ...
    {
        auto weakEngine = te::Engine::WeakRef (&engine);
        ...
        fc->launchAsync (FileBrowserComponent::openMode + FileBrowserComponent::canSelectFiles,
                         [fc, weakEngine, callback = std::move (fileChosenCallback)] (const FileChooser&)
                         {
                             const auto f = fc->getResult();

                             if (f.existsAsFile())
                                 if (auto* liveEngine = weakEngine.get())
                                     liveEngine->getPropertyStorage().setDefaultLoadSaveDirectory ("pitchAndTimeExample", f.getParentDirectory());
