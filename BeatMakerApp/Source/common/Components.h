...
public:
        ...
        : edit (e), selectionManager (s)
    {
        ...
        showRecordControls.referTo (state, IDs::showRecordControls, um, true);

        viewX1.referTo (state, IDs::viewX1, um, 0s);
        viewX2.referTo (state, IDs::viewX2, um, 60s);
        viewY.referTo (state, IDs::viewY, um, 0);
        trackHeight.referTo (state, IDs::trackHeight, um, 52.0);
    }
    ...
    CachedValue<bool> showMasterTrack, showGlobalTrack, showMarkerTrack, showChordTrack, showArrangerTrack,
                      drawWaveforms, showHeaders, showFooters, showMidiDevices, showWaveDevices, showRecordControls;
    
    CachedValue<te::TimePosition> viewX1, viewX2;
    CachedValue<double> viewY, trackHeight;
...
protected:
    ...
    void updateMouseCursorForLocalX (int x, bool shiftDown);
    bool isNearClipStartHandle (int x) const;
    bool isNearClipEndHandle (int x) const;

    EditViewState& editViewState;
    te::Clip::Ptr clip;
    DragMode dragMode = DragMode::none;
    double dragOriginalStartSeconds = 0.0;
    double dragOriginalEndSeconds = 0.0;
};
...
private:
    ...
    bool resizingLaneHeight = false;
    int resizeDragStartScreenY = 0;
